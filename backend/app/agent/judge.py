"""正确性裁判（工单 0028 阶段一 / ADR 0005「第 8 层」正确性裁决，0030 修订触发闸）。

在「改动代码轮且磁盘确有改动」时带 diff 触发；「可疑零改动轮」（分类器判
modify_code、模型以 no_change 收尾且磁盘零改动）带空 diff 照常触发——这是
「说改好了但没改」的唯一逃逸口，由裁判读用户原话裁决（诉求要求改动 →
incomplete；「不要改」类良性诉求 → in_scope）。咨询轮、首建轮与分类缺席
（辅助模型降级）的零改动轮不触发。
        ...
        *context,
        ...
        HumanMessage(
            content=(
                ...
                "即行号定位，直接引用，不要自己数行）：\n"
                f"{diff_text or '（无可解码的文本改动：本轮磁盘零改动，或改动均不可解码）'}"
            )
        ),
    ]
